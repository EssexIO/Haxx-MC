			...
			if (args.length < 2 || args.length > 2) {
				...
				if(args[0].equalsIgnoreCase("add")) {
					if(!customConfig.getBoolean(args[1] + ".Whitelist") == true) {
						...
						sender.sendMessage("§b" + args[1] + " §fadded to whitelist.");
					return true;
					} else {
						sender.sendMessage("§b" + args[1] + " §fis already on the whitelist!");
						return false;
					}
				}
				...
				if(!customConfig.getBoolean(args[1] + ".Whitelist") == false) {
				    ...
				    saveCustomYml(customConfig, PlayerDB);
				    sender.sendMessage("§b" + args[1] + " §fremoved from whitelist.");
				    return true;
				} else {
					sender.sendMessage("§b" + args[1] + " §fisn't on the whitelist!");
				}
				}
			}
		}
